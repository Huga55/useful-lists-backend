...
const authTest = (app: Express) => {
  describe.skip("Register", () => {
    ...
    it("POST /auth/register -> 201 create new user, tokens in response", async () => {
  ...
  describe("Login", () => {
    ...
    it.skip("POST /auth/login -> 401, email or password are invalids", async () => {
      const data = {
        email: "email@mail.ru",
        password: "123456789",
      };
    ...
    it.skip("POST /auth/login -> 400, body parameters are invalids", async () => {
      const response = await request(app)
        .post(endpoint)
        .expect("Content-Type", /json/)
        .expect(400);
